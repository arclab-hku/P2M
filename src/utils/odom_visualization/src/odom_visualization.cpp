...
void odom_callback(const nav_msgs::Odometry::ConstPtr &msg)
{
  ...
  if (tf45)
  {
    ...
    tf::Transform transformNone;
    transformNone.setOrigin(tf::Vector3(pose(0), pose(1), pose(2)));
    colvec y90 = zeros<colvec>(3);
    y90(0) = -90.0 * M_PI / 180;
    y90(2) = 180 * M_PI / 180;
    colvec q901 = R_to_quaternion(ypr_to_R(y90));
    transformNone.setRotation(tf::Quaternion(q901(1), q901(2), q901(3), q901(0)));
    ...
    broadcaster->sendTransform(tf::StampedTransform(transformNone, msg->header.stamp, string("world"), drone_s));
  }
}
